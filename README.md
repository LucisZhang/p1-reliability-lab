...
**Scale honesty.** This is a correctness lab, not a throughput benchmark. The captured base-path
failure cases are deliberately small enough for exhaustive diffs, while the broker artifacts
record only the bounded workloads stated above. There is no production-throughput, terabyte-table,
long-duration, cross-cloud, recovery-time, freshness, or availability-SLO claim.
