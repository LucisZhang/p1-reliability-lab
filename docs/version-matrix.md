...
| Prometheus JMX Exporter Java agent | 0.20.0 | Debezium connector metrics on `:9405/metrics` | Attached inside the existing Connect JVM to avoid remote JMX/RMI. This is the exact Maven-Central-published agent version resolved by the pinned dependency plugin; newer release-page-only artifacts are deliberately avoided because the benchmark VM could not reach GitHub release assets. Evidence: `showcase/logs/phase-b4-broker-up-20260820T145359Z.log`. |
...
| Apache Avro Java | 1.12.0 in the Flink job; Confluent-managed 1.11.5 in the Connect plugin | Flink Path B consumer and Iceberg writer | Iceberg 1.10.0 calls the Avro 1.12 `timestampNanos()` API. The explicit Flink-job pin prevents Confluent's nearer 1.11.5 dependency from replacing it; the isolated Connect plugin retains Confluent 7.9.8's native closure. |
